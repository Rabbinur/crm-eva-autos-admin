...
const DailySaleReport = () => {
  ...
  // Fetch Delivery Men
  const { data: deliveryMen = [], isLoading: isDeliveryMenLoading } = useGetDeliveryMenQuery();

  // Fetch report data
  const { data: reportData = [], isLoading, isFetching } = useGetDailySaleReportQuery({
    start_date: startDate || undefined,
    end_date: endDate || undefined,
    delivery_man_id: selectedDeliveryManId || undefined,
  });
  ...
  const totals = useMemo(() => {
    return reportData.reduce(
      (acc: any, item: any) => {
        ...
        acc.totalReturned += Number(item.totalReturned || 0);
        acc.totalDamaged += Number(item.totalDamaged || 0);
    ...
    <table className="w-full text-left border-collapse border border-slate-200">
      <thead>
        <tr className="bg-[#f1f5f9] border-b border-slate-200 text-[11px] font-bold text-slate-700 uppercase tracking-tight">
          ...
          <th className="px-3 py-3 border-r border-slate-200 text-center">Settlement ID</th>
          <th className="px-3 py-3 border-r border-slate-200">Delivery Representative</th>
          <th className="px-3 py-3 border-r border-slate-200 text-center">Loading Sheet ID</th>
          <th className="px-3 py-3 border-r border-slate-200 text-right">Total Sales</th>
          <th className="px-3 py-3 border-r border-slate-200 text-center">Returned (Pcs)</th>
          <th className="px-3 py-3 border-r border-slate-200 text-center text-rose-500">Damaged (Pcs)</th>
          <th className="px-3 py-3 text-right bg-slate-50 font-black">Profit Earned</th>
        </tr>
      </thead>
      <tbody className="divide-y divide-slate-100 text-[12px]">
        {isLoading || isFetching ? (
          <tr>
            <td colSpan={9} className="py-10 text-center text-slate-400">
              <Loader2 className="animate-spin inline-block mr-2" size={16} /> Loading sales entries...
            </td>
          </tr>
        ) : paginatedData.length === 0 ? (
          <tr className="text-center italic text-slate-400">
            <td colSpan={9} className="py-10">No data available in table</td>
          </tr>
        ) : (
          paginatedData.map((item: any, i: number) => (
            <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-3 py-2.5 border-r border-slate-200 font-mono text-slate-500 font-bold">{item.date}</td>
              <td className="px-3 py-2.5 border-r border-slate-200 text-center font-mono text-[#001f3f] font-semibold">{item.settlementId}</td>
              <td className="px-3 py-2.5 border-r border-slate-200 font-medium text-slate-800">{item.deliveryMan}</td>
              <td className="px-3 py-2.5 border-r border-slate-200 text-center text-slate-600 font-mono">{item.loadingSheetId}</td>
              <td className="px-3 py-2.5 border-r border-slate-200 text-right font-mono text-slate-700 font-medium">{formatCurrency(item.totalSales)}</td>
              <td className="px-3 py-2.5 border-r border-slate-200 text-center font-semibold text-blue-600">{item.totalReturned} Pcs</td>
              <td className="px-3 py-2.5 border-r border-slate-200 text-center font-semibold text-rose-500">{item.totalDamaged} Pcs</td>
              <td className="px-3 py-2.5 text-right font-mono text-emerald-600 font-black bg-slate-50">{formatCurrency(item.profit)}</td>
            </tr>
          ))
        )}
        ...
        {!isLoading && !isFetching && reportData.length > 0 && (
          <tr className="bg-slate-50 font-bold text-[12px] text-slate-800 border-t-2 border-slate-300">
            <td colSpan={5} className="px-3 py-3 text-center border-r border-slate-200">Total</td>
            <td className="px-3 py-3 text-right border-r border-slate-200 font-mono text-slate-700">{formatCurrency(totals.totalSales)}</td>
            <td className="px-3 py-3 text-center border-r border-slate-200 font-semibold text-blue-700">{totals.totalReturned} Pcs</td>
            <td className="px-3 py-3 text-center border-r border-slate-200 font-semibold text-rose-600">{totals.totalDamaged} Pcs</td>
            <td className="px-3 py-3 text-right font-mono text-[#001f3f] font-black">{formatCurrency(totals.profit)}</td>
          </tr>
        )}
      </tbody>
    </table>
  ...
  if (isPrinting) {
    return (
      <PrintLayout
        title="Daily Sale Settlements Report"
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 font-sans bg-slate-50/50 min-h-screen space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Daily Sale Report</h1>
          <p className="text-xs text-slate-400 font-semibold">List and verify finalized settlement records, including returned/damaged products count and profit margins</p>
        </div>
        {reportData.length > 0 && (
          <button
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          ...
          <div className="md:col-span-5 space-y-1.5">
            <label className="text-xs font-bold uppercase text-slate-400 tracking-wider">Delivery Representative</label>
            <select
